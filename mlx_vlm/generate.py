...
from mlx_lm.models.cache import (
    ...
    make_prompt_cache as mlx_lm_make_prompt_cache,
    trim_prompt_cache,
)
...
def _extract_text_tokens(input_ids: mx.array, model_config) -> mx.array:
    """Remove vision/audio placeholder tokens from input_ids for draft model prefill."""
    exclude = set()
    ...
    text_ids = [tid for tid in ids_list if tid not in exclude]
    return mx.array(text_ids, dtype=mx.uint32)


def speculative_generate_step_vlm(
    input_ids: mx.array,
    model: nn.Module,
    ...
    *,
    ...
    draft_cache: Optional[List[Any]] = None,
    ...
    **kwargs,
    ...
    """Speculative decoding for VLMs with a text-only draft model.

    The main model (VLM) handles the full vision + text prefill. The draft model
    (text-only LM from mlx-lm) generates candidate tokens during decode, which the
    main model verifies in a single forward pass.

    Args:
        input_ids: Input token IDs (may include vision placeholder tokens).
        model: The VLM model (has .language_model and .get_input_embeddings).
        draft_model: A text-only draft LM from mlx-lm.
        pixel_values: Vision inputs (images/video), or None for text-only.
        mask: Attention mask for vision inputs, or None.
        num_draft_tokens: Number of draft tokens per speculative step.
        max_tokens: Maximum tokens to generate.

    Yields:
        (token, logprobs, from_draft): Token value, log probabilities, and whether
        the token was generated by the draft model.
    """
    # --- Setup ---
    if sampler is None:
    ...
    processors = make_logits_processors(
        ...
        kwargs.pop("repetition_context_size", 20),
    )
    ...
    if prompt_cache is None:
        model_cache = cache.make_prompt_cache(
            ...
            max_kv_size=None,  # speculative decoding requires trimmable caches
        )
    else:
        model_cache = prompt_cache

    if draft_cache is None:
        draft_cache = mlx_lm_make_prompt_cache(draft_model)

    # Validate trimmability
    if not can_trim_prompt_cache(model_cache):
        ...
        raise ValueError(
            ...
            f"Models with RotatingKVCache (e.g., Gemma 4) are not supported."
        )

    # KV quantization for the main model only
    ...
    # State for logits processors
    prev_tokens = None
    # State for cross-attention / encoder outputs from VLM
    decode_kwargs = {}
    ...
    def _step_main(y, n_predict=1):
        """Run main VLM language model forward pass."""
        nonlocal prev_tokens, decode_kwargs
        with mx.stream(generation_stream):
            outputs = model.language_model(y[None], cache=model_cache, **decode_kwargs)
            logits = outputs.logits[:, -n_predict:, :]
            quantize_cache_fn(model_cache)

            # Capture cross-attention state for next decode step
            if outputs.cross_attention_states is not None:
                decode_kwargs = {"cross_attention_states": outputs.cross_attention_states}
            elif outputs.encoder_outputs is not None:
                decode_kwargs = {"encoder_outputs": outputs.encoder_outputs}
            else:
                decode_kwargs = {}

            if processors:
                out_y, out_logprobs = [], []
                trim_y = y if n_predict <= 1 else y[: -(n_predict - 1)]
                for i in range(n_predict):
                    prev_tokens = (
                        mx.concatenate([prev_tokens, trim_y])
                    ...
                    if i < n_predict - 1:
                        trim_y = mx.array([y[-(n_predict - 1 - i)]], dtype=y.dtype)
    ...
    def _prefill_main(input_ids_pf, pixel_values_pf, mask_pf):
        """Prefill the main VLM model with vision + text."""
        nonlocal decode_kwargs
        embedding_output = model.get_input_embeddings(
            input_ids_pf, pixel_values_pf, mask=mask_pf, **kwargs
        )
        ...
        if prefill_step_size is not None and inputs_embeds.shape[1] > prefill_step_size:
            while inputs_embeds.shape[1] > 1:
                n = min(prefill_step_size, inputs_embeds.shape[1] - 1)
                model.language_model(
                    inputs=ids[:, :n],
        ...
        # Final prefill step: get first token
        outputs = model.language_model(
            ids, inputs_embeds=inputs_embeds, cache=model_cache, **pf_kwargs
        )
    ...
    def _prefill_draft(text_tokens):
        """Prefill the draft model with text-only tokens."""
        y = text_tokens.astype(mx.uint32)
        while y.size > 1:
            n = min(prefill_step_size or 512, y.size - 1)
            draft_model(y[:n][None], cache=draft_cache)
    ...
    # --- Prefill both models ---
    with mx.stream(generation_stream):
        # Extract text-only tokens for draft model (remove image placeholders)
        text_tokens = _extract_text_tokens(input_ids, model.config)
        _prefill_draft(text_tokens)
        y, logprobs = _prefill_main(input_ids, pixel_values, mask)
    ...
    try:
        while True:
            ...
            draft_tokens = _draft_generate(y, num_draft)
            y_combined = mx.concatenate([y, draft_tokens])
            tokens, all_logprobs = _step_main(y_combined, num_draft + 1)
            ...
            n = 0
            while n < num_draft:
                tn, dtn, lpn = tokens_list[n], draft_tokens_list[n], all_logprobs[n]
                if tn != dtn:
                    break
                n += 1
                ntoks += 1
            ...
            if ntoks < max_tokens:
                ntoks += 1
                yield tokens_list[n], all_logprobs[n], False
            ...
            # If all draft tokens accepted, include the last draft token
            # in the next draft step (draft model hasn't processed it yet)
            if n == num_draft:
                draft_y = mx.concatenate(
                    [mx.array(draft_tokens_list[-1:], mx.uint32), draft_y]
                )
                _step_draft(mx.array(draft_tokens_list[-1:], mx.uint32))
...
def stream_generate(
    ...
    **kwargs,
    ...
    # Extract draft model if provided
    draft_model = kwargs.pop("draft_model", None)
    num_draft_tokens = kwargs.pop("num_draft_tokens", 3)
    ...
    if "prompt_cache" not in kwargs:
        kwargs["prompt_cache"] = cache.make_prompt_cache(
            model.language_model,
            max_kv_size=kwargs.get("max_kv_size", None) if draft_model is None else None,
        )
    ...
    with wired_limit(model, [generation_stream]):
        ...
        # Route to speculative decoding if draft model provided and cache is trimmable.
        # Note: RotatingKVCache reports trimmable when empty but becomes non-trimmable
        # at capacity. Check for its presence as a conservative guard.
        _has_rotating = any(
            type(c).__name__ == "RotatingKVCache" for c in tracked_cache
        )
        use_speculative = (
            draft_model is not None
            and not _has_rotating
            and can_trim_prompt_cache(tracked_cache)
        )
        if use_speculative:
            kwargs.pop("max_kv_size", None)  # incompatible with speculative
            kwargs.pop("prompt_cache", None)  # passed explicitly below
            spec_gen = speculative_generate_step_vlm(
                input_ids, model, draft_model, pixel_values, mask,
                num_draft_tokens=num_draft_tokens,
                prompt_cache=tracked_cache,
                **kwargs,
            )
            # Wrap to match (token, logprobs) interface
            gen = ((token, logprobs) for token, logprobs, _from_draft in spec_gen)
        else:
            if draft_model is not None:
                print(
                    "[speculative] Cache is not trimmable — falling back to "
                    "standard generation."
                )
